...
2. **函数模板的实例化（instantiation）**：模板实参确定之后，编译器就使用**模板实参代替相应的模板形参**，产生并编译函数模板的一个特定版本（称为函数模板的一个实例（instance））（注意：此过程中不进行常规隐式类型转换）
...
- 在步骤2中：首先匹配**函数模板的特化**，再匹配非指定特殊的函数模板（通用模板）
...
| ------------------------ | ---------------------- | ---------------------------- |
| 是否自动推导出数组大小？ | ✅                      | ✅                            |
| 使用方法                 | 直接传数组             | 显式取地址，使用 `&arr` 形式 |



#### 有非类型模板形参的显式模板特化

模板形参列表种，要***保留* 非类型模板形参**
...
要求： 在函数模板或类模板成员函数的调用点，相应函数的定义对编译器必须是可见的。简述：**模板定义在调用点可见**。

实现方式： 

- 模板的**定义**必须都在头文件中完成，
- 模板类的使用（包括对象声明），在头文件或同一 `.cpp` 中包含了定义的头文件
